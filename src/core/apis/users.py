...
def create_user(user_data: UserSchema) -> UserResponse:
    """Create a new user"""
...
def get_user(uid: str) -> UserResponse:
    """Get user by ID"""
    user: Optional[User] = user_repo.get_by_id(uid=uid)
    if not user:
...
def update_user(uid: str, user_data: UserSchema) -> UserResponse:
    """Update user"""
    existing_user: Optional[User] = user_repo.get_by_id(uid=uid)
